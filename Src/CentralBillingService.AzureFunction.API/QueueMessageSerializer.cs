...
{
    public static T? Deserialize<T>(string message, JsonSerializerOptions? options = null)
    {
        // 1. Try the message as-is (plain JSON).
        if (TryDeserialize<T>(message, options, out T? direct))
            return direct;

        // 2. Fall back to Base64-decoding then JSON.
        if (TryFromBase64(message, out string decoded) &&
            TryDeserialize<T>(decoded, options, out T? fromBase64))
            return fromBase64;
